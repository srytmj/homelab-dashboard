...
`GitProjectsService`'s pull/rebuild belongs on that primary-only list too, even though it isn't a `DockerService`: `pullAndRebuild()` always shells out against this daemon's own `docker.sock` and `config.gitProjectsRoot`, never a remote host's, regardless of which configured host the tracked container's `dockerHost` says it actually runs on. `GitProjectModal.tsx` states this limitation directly in the tracking form. Real multi-host support would mean resolving each project to its own `SSH_TARGETS` entry and running `git`/`docker` there instead — that doesn't exist yet, so don't wire `getDockerServiceForContainer`-style host resolution into `GitProjectsService` as a shortcut; it would silently rebuild against the wrong daemon.
